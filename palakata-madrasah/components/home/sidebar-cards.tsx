    ...
    <aside className="flex flex-col gap-5">
      ...
      <SideCard title={t("Founder & Patron", "প্রতিষ্ঠাতা ও পৃষ্ঠপোষক")}>
        <div className="px-5 py-5 text-center">
          ...
          <p className="font-display text-h4 text-green-dark">
            {t(site.founder.name.en, site.founder.name.bn)}
          </p>
          <p className="mt-1 text-note italic text-muted-ink">
      ...
      <SideCard title={t("Online Admission", "অনলাইন ভর্তি")}>
        <div className="px-5 py-6 text-center">
          ...
          <p className="mb-4 text-copy text-muted-ink">
            {t(
          ...
          {importantLinks.map((l) => (
            <li key={l.href} className="border-b border-line last:border-b-0">
              <a
                ...
                className="flex items-center gap-2.5 px-5 py-2.5 text-ui text-ink transition-all hover:bg-secondary hover:pl-7 hover:text-orange"
              >
                <ChevronRight
...
function SideCard({
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-line bg-white shadow-sm">
      <h3 className="font-display border-b-2 border-green-dark bg-gradient-to-b from-white to-[#f7f9fc] p-3.5 text-center text-h4 text-green-dark">
        {title}
      </h3>
      {children}
    </div>
